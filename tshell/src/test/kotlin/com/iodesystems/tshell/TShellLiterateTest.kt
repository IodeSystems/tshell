...
accepted but ignored. Use `help()` to discover what's available.
  ...
  @Test fun `generate README from literate examples`() {
    val readme = buildString {
      ...
      appendLine()
      appendLine("tshell connects to any MCP server and collapses its tools into the same")
      appendLine("`eval` call. Write your tools in Go, Python, TypeScript, Rust — whatever")
      appendLine("fits your stack. The LLM sees one tool; tshell handles the plumbing.")
      appendLine()
      appendLine("```")
      appendLine("LLM")
      appendLine(" │")
      ...
      appendLine("```")
      appendLine()
      appendLine("**Why this matters:** each MCP tool you add inflates the LLM's context with")
      appendLine("schema, descriptions, and examples. A typical Playwright MCP server adds ~8KB")
      appendLine("of tool definitions. tshell replaces all of it with a single `eval` tool —")
      appendLine("commands are discovered via `help()` at runtime, not baked into the prompt.")
      appendLine("The LLM's KV cache survives when you add or remove servers.")
      appendLine()
      appendLine("```bash")
      appendLine("# Standalone MCP server")
      appendLine("tshell-cli")
      appendLine()
      appendLine("# Connect to external MCP servers — tools become namespaced commands")
      appendLine("tshell-cli --connect \"app=python my_tools.py\" --connect \"data=npx data-server\"")
      appendLine("```")
      appendLine()
      appendLine("See [`tshell-mcp/README.md`](../tshell-mcp/README.md) for the full API.")
      appendLine()
